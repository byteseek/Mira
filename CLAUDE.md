...
- For `standard` / `deep_dive` research, run `scripts/check_updates.sh
  --local-first` once (local-first with a 24h remote TTL); `quick_map` / 看一下
  tasks skip it. A blocked fetch degrades to local refs and is reported — never
