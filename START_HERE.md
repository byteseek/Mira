...
4. `standard` / `deep_dive` 研究前运行 `scripts/check_updates.sh --local-first`（local-first，24h remote TTL）只检查 freshness，不自动更新；`quick_map` / 看一下跳过。fetch 被拦就降级到 local refs 并说明，不为此提权。
