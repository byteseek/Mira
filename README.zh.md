...
| `standard`/`deep_dive` 研究前检查 freshness | `scripts/check_updates.sh --local-first` |
...
2. 否则，`standard`/`deep_dive` 研究前用 `scripts/check_updates.sh --local-first` 检查 remote freshness（local-first，24h remote TTL；`quick_map` 跳过）。fetch 被拦就降级到缓存的 local refs，不为 freshness check 提权。
