...
`critical_interaction_step` 是每个 Mira workflow 的共同出口检查。它回答：
当前任务完成到这里，是否存在一个用户回答后会显著改变研究边界、证据路径、
计算深度、readiness、thesis state、actionability boundary 或下一 route 的问题？

这一步把问答模式升级为交互式工作模式。`quick_map` 不是例外，而是最小交互
单元：默认给 1 个最高杠杆问题。`standard` / `deep_dive` / decision-grade
workflow 给 2-3 个问题或明确 handoff。只有用户明确不要反问、任务是机械转换、
或下一步已经唯一确定时，才允许 waiver。
...
- `workflow_handoff`: 当前输出已经形成 reusable view，需要询问是否升级为
  ...
  `position_review`、`portfolio_review` 或 `actionability_bridge`。
- `followup_answer_continuation`: 当前用户消息是在回答上一轮 follow-up；沿用
  上一轮的 `followup_id` / `next_route_if_answered`，不要重新当普通问题路由。
- `waive_with_reason`: 没有有用交互，或用户明确不要；必须写明
  `followup_waiver_reason`。

progressive follow-up 是其中最常见的后置交互形式。它不是普通“还有什么想问”，
而是督促用户进一步思考并触发下一层 workflow 的控制点。

默认先完成当前 route 能支持的回答，再给 1-3 个高杠杆反问。只有当研究对象、
...
如果用户下一轮回答类似“选第二个”“先看估值”“进入 expectation-map”
...
- 使用上一轮的 `followup_route_binding`、`object_anchor` 和
  `next_route_if_answered` 作为路由基础。
- 更新 `state_update_if_answered` 指向的对象，例如 `private working view`、
  `expectation_map`、`evidence_path`、`calculation_gate`、`thesis_state` 或
...
- 每个 workflow 都必须先完成 `critical_interaction_step`，选择
  ...
  `followup_answer_continuation` 或 `waive_with_reason`。不得把这一步理解为
  quick_map 的可选装饰。
- 如果 `followup_prompt_mode` 是 `light`、`standard` 或 `decision_grade`，
  内部必须记录 1-3 个问题，且每个问题都有 `route_binding`、
  `object_anchor` 和 `decision_impact`；用户可见输出只需用自然语言体现这些
  绑定，不默认暴露机器字段。
...
- 如果当前输出产生 reusable view 或 durable thesis candidate，必须考虑
...
```md
下一步最有用的问题：围绕 `[object_anchor]`，你更希望验证 `[route-bound choice]`
还是 `[route-bound choice]`？回答后我会进入 `[loop_or_skill]`，并改变
`[decision_impact]`。
```

当输出是 `quick_map` 或 `quick_answer` 时，优先使用一行版本：

```md
下一步最有用的问题：先验证 `[specific variable/source gap]` 吗？回答后我会把它接到 `[next route]`。
```

以下结构只用于内部记录、debug artifact 或正式 `routing.json`，不是默认用户
可见形态：
